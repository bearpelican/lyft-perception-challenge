...
model_path = str(Path.cwd()/'600urn-35-unet11-mini-nocrop.h5')
...
#     frame_idx = 1+i
    for frame in preds:
        ...
        binary_car_result = (frame[0]).astype('uint8')
        binary_road_result = (frame[1]).astype('uint8')
#         s_idx = frame_idx
        res.append([encode(binary_car_result), encode(binary_road_result)])
#         if frame_idx in mismatched_idxs:
#             s_idx -= 1
#         answer_key[s_idx] = [encode(binary_car_result), encode(binary_road_result)]
#         frame_idx+=1
